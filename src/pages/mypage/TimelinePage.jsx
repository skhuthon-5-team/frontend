  ...
  return (
    <div className="mt-4 overflow-hidden rounded-xl border border-border-default">
      <div className="flex items-center justify-between bg-surface-inverse px-4 py-3">
        ...
        <span className="text-xs text-text-muted">과거의 실패 vs 현재의 성공</span>
      </div>

      <div className="grid grid-cols-1 gap-6 p-5 md:grid-cols-2">
        <div className="min-w-0">
          <p className="text-xs font-bold text-text-muted">{report.before.label}</p>
          <ul className="mt-2 flex flex-col gap-1.5">
            {report.before.points.map((p) => (
              <li key={p} className="text-sm text-text-muted">{p}</li>
            ))}
          </ul>
        </div>
        <div className="min-w-0">
          <p className="text-xs font-bold text-text-strong">{report.after.label}</p>
          <ul className="mt-2 flex flex-col gap-1.5">
            {report.after.points.map((p) => (
              <li key={p} className="text-sm text-text-default">{p}</li>
            ))}
          </ul>
        </div>
      </div>
        ...
        {item.type === "success" ? (
          <div className="rounded-2xl border border-border-default p-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-text-muted">최종 업데이트: {item.updatedAt}</span>
            </div>
            <h3 className="mt-3 text-lg font-bold text-text-strong">{item.title}</h3>
...
export default function TimelinePage() {
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-text-strong">나의 성장 타임라인</h1>
        <Button variant="primary" className="flex items-center gap-1.5 px-4 py-2 text-sm">
          <Download size={15} />
          성장 리포트 PDF
        </Button>
      </div>
        ...
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl bg-surface-subtle p-5">
            ...
            <p className="mt-2 text-2xl font-bold text-text-strong">
              {s.value}
              <span className="ml-1 text-sm font-medium text-text-muted">{s.unit}</span>
            </p>
          </div>
        ))}
        <div className="rounded-xl bg-surface-inverse p-5">
          <p className="text-xs text-text-muted">{statusCard.label}</p>
          <p className="mt-2 text-base font-bold text-text-on-inverse">{statusCard.value}</p>
