    ...
    <footer className="border-t border-border-default bg-surface-base">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <div className="flex flex-col gap-10 md:flex-row md:justify-between">
          <div className="max-w-sm">
            ...
            <p className="mt-3 text-sm text-text-muted">
              우리는 실패를 숨기지 않습니다. 실패는 성공으로 가는 가장 빠른 지름길임을 믿습니다.
            </p>
          </div>
          ...
          <div className="flex gap-16">
            {linkColumns.map((column) => (
              <div key={column.title}>
                <p className="text-xs font-bold tracking-wider text-text-strong">{column.title}</p>
                <ul className="mt-4 flex flex-col gap-3">
                  {column.links.map((link) => (
                    <li key={link}>
                      <a href="#" className="text-sm text-text-muted transition hover:text-text-strong">
                        {link}
                      </a>
                    </li>
                  ))}
        ...
        <div className="mt-12 flex flex-col gap-4 border-t border-border-default pt-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-text-muted">© 2026 Failure Log. All rights reserved.</p>
